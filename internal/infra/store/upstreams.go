...
// upstreamID derives a deterministic id from (partition, match_host) so
// SetUpstream is naturally idempotent — no UUID generation needed, and
// calling it twice for the same partition+host updates rather than
// duplicates.
func upstreamID(partition, matchHost string) string {
	return partition + "\x00" + matchHost
}
...
func (s *Store) SetUpstream(ctx context.Context, u domain.Upstream) error {
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO upstreams (id, "partition", match_host, target_url, tls_skip_verify)
		VALUES (?, ?, ?, ?, ?)
		ON CONFLICT(id) DO UPDATE SET
			target_url = excluded.target_url,
			tls_skip_verify = excluded.tls_skip_verify`,
		upstreamID(u.Partition, u.MatchHost), u.Partition, u.MatchHost, u.TargetURL, boolToInt(u.TLSSkipVerify),
	)
	if err != nil {
...
func (s *Store) ListUpstreams(ctx context.Context, partition string) ([]domain.Upstream, error) {
	...
	for rows.Next() {
		var matchHost, targetURL string
		var skip int
		if err := rows.Scan(&matchHost, &targetURL, &skip); err != nil {
			return nil, fmt.Errorf("store: scan upstream row: %w", err)
		}
		out = append(out, domain.Upstream{
			Partition: partition, MatchHost: matchHost, TargetURL: targetURL, TLSSkipVerify: skip != 0,
		})
	}
	return out, rows.Err()
}
