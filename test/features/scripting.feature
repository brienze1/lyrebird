Feature: Scripting
  ...
  Scenario: The sandbox exposes no filesystem, network, or environment globals
    Given a mock named "sandboxed" matching GET path "/sandboxed" with script.respond_src "({fetch: typeof fetch, proc: typeof process, req: typeof require, xhr: typeof XMLHttpRequest, dirname: typeof __dirname, glob: typeof global})" that responds 200
    When I send a GET request to "/sandboxed" on the data plane with host "example.local"
    Then the response status is 200
    And the response body is '{"dirname":"undefined","fetch":"undefined","glob":"undefined","proc":"undefined","req":"undefined","xhr":"undefined"}'
  ...
  Scenario: An infinite-loop match_src script fails safe, is recorded, and the server keeps serving
    Given a mock named "loopmatch" matching GET path "/loopmatch" with script.match_src "while(true){}" that responds 200
    And a mock named "ping2" matching GET path "/ping2" that responds 200 with body "pong"
    When I send a GET request to "/loopmatch" on the data plane with host "example.local"
    Then the response status is 500
    And the recorded traffic for that request has decision "script_failed"
    When I send a GET request to "/ping2" on the data plane with host "example.local"
    Then the response status is 200
    And the response body is "pong"
