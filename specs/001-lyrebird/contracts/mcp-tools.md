...
| `list_examples` | `{ query? }` | Recipe index (AWS/GCP/third-party as plain HTTP). Content only. | FR-022 |
| `get_example` | `{ id }` | One recipe → ready-to-adapt `create_mock` payload. | FR-022 |
...
| `get_traffic` | `{ id }` | Full request + response (decrypted). | FR-002, FR-021 |
